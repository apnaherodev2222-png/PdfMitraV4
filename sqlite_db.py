...
class Database:
    ...
    def init(self):
        ...
        with self._lock:
            ...
            # Groq was removed from the application; purge any old provider rows.
            self.conn.execute("DELETE FROM ai_keys WHERE provider='groq'")
            self.conn.commit()
    ...
    def add_ai_key(self, provider, key, added_by=None):
        ...
        if not provider or not key:
            return False
        # Exact provider+key uniqueness is enforced both in code and by SQLite.
        existing=self.conn.execute("SELECT id FROM ai_keys WHERE provider=? AND api_key=? LIMIT 1",(provider,key)).fetchone()
        if existing:
            return False
        try:
            self._execute("INSERT INTO ai_keys(provider,api_key,status,added_by,added_at,fail_count) VALUES(?,?,?,?,?,0)",(provider,key,'active',added_by,iso(utcnow())),True)
    ...
    def ai_key_by_value_any(self, key):
        key=(key or '').strip()
        if not key:
            return None
        r=self._execute("SELECT * FROM ai_keys WHERE api_key=? ORDER BY id LIMIT 1",(key,)).fetchone()
    ...
    def get_active_ai_key(self, provider):
        # FIX_SCAN_1: don't touch last_used here — this is also called by pure
        # availability checks (_ai_available/_ai_key, e.g. every menu render
        # via ui.py) that never actually make an API call with the key.
        # Bumping last_used on a mere check starves this key from ever being
        # picked as "least recently used" by the real rotation path below.
        # last_used is now only updated in mark_ai_key_ok/mark_ai_key_failed,
        # i.e. after the key was genuinely attempted.
        with self._lock:
    ...
    def get_active_ai_keys(self, provider, limit=None):
        ...
        # FIX_SCAN_1: don't bump last_used for every candidate returned here —
        # only the key actually attempted should move to the back of the
        # rotation. Callers mark real usage via mark_ai_key_ok/mark_ai_key_failed.
    ...
    def mark_ai_key_failed(self, key_id):
        # FIX_SCAN_1: bump last_used here — this runs after a real attempt,
        # unlike the old fetch-time update in get_active_ai_key(s).
        with self._lock:
            now=iso(utcnow())
            self.conn.execute("UPDATE ai_keys SET fail_count=fail_count+1, last_used=? WHERE id=?",(now,int(key_id)))
    ...
    def mark_ai_key_ok(self, key_id):
        # FIX_SCAN_1: bump last_used here too, so a successful real attempt
        # — not a mere availability check — is what moves a key to the back
        # of the least-recently-used rotation.
        self._execute("UPDATE ai_keys SET fail_count=0, last_used=? WHERE id=?",(iso(utcnow()),int(key_id)),True)

    def touch_ai_key_last_used(self, key_id):
        """FIX_SCAN_1: record that a key was genuinely attempted (call made),
        without touching fail_count/status. Used for non-auth exceptions
        (network error, timeout, 5xx) so a flaky-but-not-actually-bad key
        still rotates to the back of the queue instead of being retried
        first again on every subsequent request."""
        self._execute("UPDATE ai_keys SET last_used=? WHERE id=?",(iso(utcnow()),int(key_id)),True)
    ...
    def settings(self,user_id):
        ...
        r=self._execute("SELECT * FROM settings WHERE user_id=?",(user_id,)).fetchone()
        if r is None:
            # FIX_PM_8: _default_settings returns font_size/page_size keys but
            # every caller reads settings["size"] / settings["page"]. Returning
            # the raw default caused KeyError on race/fallback paths.
            d = self._default_settings(user_id)
            return {'font':d['font'],'size':d['font_size'],'page':d['page_size'],
                    'margin':d['margin'],'line_spacing':d['line_spacing'],
                    'alignment':d['alignment'],'bold_title':d['bold_title'],
                    'title_size':d['title_size'],'header':d['header'],
                    'footer':d['footer']}
