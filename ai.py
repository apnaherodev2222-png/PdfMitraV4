...
# FIX_PM_19: fontTools cmap walk is 20-100ms. HINDI_FONT_PATH is set once
# at boot and never changes, so memoize the Latin-coverage check.
_HINDI_LATIN_CACHE: Optional[bool] = None

def _hindi_supports_latin() -> bool:
    global _HINDI_LATIN_CACHE
    if _HINDI_LATIN_CACHE is None:
        _HINDI_LATIN_CACHE = bool(
            HINDI_FONT_AVAILABLE
            and HINDI_FONT_PATH.exists()
            and font_supports_latin(str(HINDI_FONT_PATH))
        )
    return _HINDI_LATIN_CACHE

def _available_font_catalog() -> List[Dict]:
    # FIX_PM_5: HINDI_FONT_AVAILABLE only means the file downloaded — it says
    # nothing about Latin glyph coverage. NotoSansDevanagari has no Latin
    # letters, so marking it latin=True made _local_smart_font pick it for
    # mixed-script text, producing tofu boxes for English characters.
    _hindi_latin = _hindi_supports_latin()
    catalog = []
    if HINDI_FONT_AVAILABLE and HINDI_FONT_PATH.exists():
        catalog.append({
            "name": "NotoHindi",
            "path": str(HINDI_FONT_PATH),
            "dev": True, "latin": _hindi_latin,
        })
    catalog.append({"name": "Helvetica", "path": None, "dev": False, "latin": True})
    for f in db.fonts():
        path = f.get("path")
        catalog.append({
...
def ai_key_candidates(provider: str) -> List[str]:
    """Public wrapper used by handlers; avoids wildcard-import omission of underscore names."""
...
def detect_ai_provider(key: str) -> tuple[Optional[str], str]:
    ...
    for other in ("gemini", "openrouter", "mistral"):
        if other not in ordered:
            ordered.append(other)
    last_msg = "Key kisi bhi supported AI provider (Gemini / OpenRouter / Mistral) se match nahi hui."
...
def _ai_model(provider: str) -> str:
    return {"gemini": GEMINI_MODEL, "mistral": MISTRAL_MODEL, "openrouter": OPENROUTER_MODEL}.get(provider, GEMINI_MODEL)
...
def _rotate_on_auth_failure(provider: str, key: str, exc: Exception):
    """Record that this key was actually attempted, so the next call to
    _ai_key_candidates() rotates to a different key.

    FIX_SCAN_1: previously this only touched the DB (via mark_ai_key_failed)
    for auth/rate-limit errors (401/403/429), and last_used was otherwise
    bumped eagerly at fetch time in get_active_ai_key(s) regardless of
    whether the key was ever actually used. Now that fetch-time bump is
    gone (a pure _ai_available() check no longer corrupts rotation), so
    every genuine attempt must record itself here: auth/rate-limit errors
    still escalate fail_count (and eventually mark the key 'down'), while
    other transient errors (network, timeout, 5xx) just rotate the key to
    the back of the queue via touch_ai_key_last_used without penalizing
    its health status.
    """
    ...
    if not row:
        return
    msg = str(exc)
    if msg.startswith("HTTP 401") or msg.startswith("HTTP 403") or msg.startswith("HTTP 429"):
        db.mark_ai_key_failed(row["id"])
    else:
        db.touch_ai_key_last_used(row["id"])
