...
async def rich_replace(chat_id, message_id, blocks):
    ...
    # FIX_PM_94 v2: Try in-place edit first (cheapest, keeps position,
    # no duplicate visible). If edit fails, send the new message FIRST
    # so that if the send also fails the old message is still intact
    # for the PTB fallback path to edit. Only delete the old message
    # AFTER the new send is confirmed successful.
    if not RICH_AVAILABLE:
        raise RichMessageError("rich_message.py unavailable")
    block_list = list(blocks)
    try:
    ...
    new_resp = await _send_with_retry(chat_id, block_list)
    try:
