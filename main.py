...
def build_app():
    if not TOKEN or TOKEN == "PASTE_YOUR_BOT_TOKEN_HERE":
        raise RuntimeError("TOKEN missing: .env me TOKEN set karo.")
    if OWNER_ID <= 0:
        raise RuntimeError("OWNER_ID missing/invalid: .env me apna numeric Telegram owner ID set karo.")
...
def main():
    ...
    build_app().run_polling(drop_pending_updates=True, close_loop=False)

if __name__ == "__main__":
    keep_alive()
    # FIX_PM_95: cap restart attempts so a permanently broken config (e.g. a
    # bad TOKEN) doesn't spin the process forever.
    _restarts = 0
    _MAX_RESTARTS = 20
    while _restarts < _MAX_RESTARTS:
        try:
            main()
            break
            ...
            _restarts += 1
            logger.exception("Bot crashed (attempt %s/%s): %s",
                              _restarts, _MAX_RESTARTS, exc)
            time.sleep(5)
    else:
        logger.error("Giving up after %s restarts.", _MAX_RESTARTS)
