...
async def safe_alert(q, text: str):
    """FIX_PM_40: show_alert even after the top-level q.answer() was
    already called. If Telegram rejects the second answerCallbackQuery,
    fall back to a plain reply message so the user still sees the text."""
    try:
        await q.answer(text, show_alert=True)
        return
    except BadRequest as exc:
        msg = str(exc).lower()
        # Callback expired / too old — nothing we can do.
        if "query is too old" in msg or "query id is invalid" in msg:
            return
    except Exception:
        pass
    # Fallback: send the alert as a normal chat message.
    try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == db.get_owner_id():
        blocks.append(button_row([rich_callback_button("👑 Admin Panel", "admin_panel", "danger")]))
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu":
        ...
        if not RICH_AVAILABLE:
            ...
            if user_id == db.get_owner_id():
                _blocks.append(button_row([rich_callback_button("👑 Admin Panel", "admin_panel", "danger")]))
        ...
        # preserve _gate_cache across user_data.clear().
        await clear_workflow_ui(context, chat_id)
        await safe_delete_message(q.message)
        _safe_clear_user_data(context)
        ...
        if len(text) > MAX_TEXT_CHARS_PER_BATCH:
            await safe_alert(q, "Text limit reached."); return
        ...
        provider = _ai_provider()
        if not _ai_available(provider):
            await safe_alert(q, f"{AI_PROVIDERS[provider]} AI configured nahi hai."); return
        ...
        provider = _ai_provider(user_id)
        if not _ai_available(provider):
            await safe_alert(q, "AI configured nahi hai."); return
        ...
        if provider not in AI_PROVIDERS:
            await safe_alert(q, "Invalid provider"); return
        ...
        if user_id != db.get_owner_id():
            await safe_alert(q, "Owner only"); return
        lang=data.split(":",1)[1]
        context.user_data["action"]="font_upload"
        ...
        text=context.user_data.get("pending_text", "").strip()
        if not text:
            await safe_alert(q, "Text session expire ho gayi."); return
        a=analyze_text_language(text)
        await safe_edit_message_text(q, 
