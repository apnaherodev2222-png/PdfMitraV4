...
async def finish_images_callback(q,context,user_id):
    ...
    try:
        ...
        if not RICH_AVAILABLE: msg=await _image_result_fallback()
        else: msg=await rich_send_or(status_message.chat_id,[heading("🎉 PDF ready"),paragraph("Aap ek aur PDF bana sakte ho."),*_rich_keyboard_blocks(_markup)],_image_result_fallback)
        ...
        _mid = rich_message_id(msg) or getattr(msg, "message_id", None)
        if _mid:
            context.user_data["last_result_message_id"] = _mid
    except Exception:
        if not delivered:
            db.release_pdf_slot(user_id)
...
async def finish_merge_callback(q,context,user_id):
    ...
    try:
        ...
        if not RICH_AVAILABLE: msg=await _merge_result_fallback()
        else: msg=await rich_send_or(status_message.chat_id,[heading("🎉 PDF merge complete"),paragraph("Aap ek aur PDF merge kar sakte ho."),*_rich_keyboard_blocks(_markup)],_merge_result_fallback)
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX_PM_60: successful channel verification rich card with plain PTB fallback.
        ...
        if not RICH_AVAILABLE: await _admin_cleanup_fallback()
            ...
            _blocks=[heading("🧹 Cleanup Complete"),paragraph("Old temporary files cleanup complete."),*_rich_keyboard_blocks(_markup)]
        ...
        if not RICH_AVAILABLE: await _admin_premium_fallback()
            ...
            _blocks=[heading("💎 Premium Management"),paragraph("Premium manage karne ke liye:"),paragraph("/premium USER_ID\n/free USER_ID"),*_rich_keyboard_blocks(_markup)]
